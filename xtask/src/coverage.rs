...
/// `Cargo.toml`. An unreadable manifest is a hard error: degrading
/// to the baseline would silently drop the project's exclusions and
/// fail the run on hardware-bound files, which reads as a coverage
/// regression rather than the config problem it is. A pattern that
/// would match every file is likewise a hard error -- it silently
/// neuters the gate, which is never intended.
fn ignore_regex() -> Result<String, String> {
    let path = workspace_root().join("Cargo.toml");
    let manifest = fs::read_to_string(&path).map_err(|e| {
        format!(
            "cannot read {} for [workspace.metadata.coverage]: {e}",
            path.display()
        )
    })?;
...
pub fn coverage_check() -> Result<CoverageResult, String> {
    ...
    // The literal check in `validate_ignore_patterns` can only catch
    // spellings it knows. This catches every one of them: whatever
    // the pattern was, if the run ended up measuring nothing then the
    // gate is vacuous and must not report success.
    let measured = json["data"][0]["files"].as_array().map_or(0, Vec::len);
    if measured == 0 || total == 0 {
        return Err(format!(
            "coverage measured {measured} files ({total} lines) -- the gate \
             is vacuous. Check [workspace.metadata.coverage] ignore in the \
             root Cargo.toml; a pattern there is excluding everything."
        ));
    }
