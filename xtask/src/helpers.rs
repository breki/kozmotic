use std::fmt;
// Some helpers below are marked `#[allow(dead_code)]`. They come
// from the rustbase template and back xtask modules this project
// does not carry (clean-cache, backfeed, feedback). They are kept
// verbatim, with their tests, so future `/template-sync` runs see
// no local drift -- delete them only if the template does.
...
#[allow(dead_code)] // template helper; see module note
...
pub(crate) fn temp_scratch(label: &str) -> PathBuf {
    ...
    let dir = std::env::temp_dir()
        .join(format!("kozmotic-xtask-{label}-{pid}-{seq}"));
