...
//! `TODO.md` without loading the whole (large) file into an
//! editor's context.
//!
...
/// # Errors
///
/// Returns an error if `TODO.md` cannot be read/written, a
...
fn add(
    ...
    if slug_exists(&content, slug) {
        return Err(format!(
            "slug '{slug}' already exists in TODO.md; pick another"
        ));
    }
    let label = if issue {
...
fn add_pending(content: &str, bullet: Vec<String>) -> Result<String, String> {
    ...
    let (start, end) = section_body(&lines, "## Pending")
        .ok_or("TODO.md has no '## Pending' section")?;
    let last_content =
...
fn move_to_done(
    ...
    let (d_start, _) = section_body(&lines, "## Done")
        .ok_or("TODO.md has no '## Done' section")?;
