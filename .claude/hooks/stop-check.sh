...
input="$(cat)"
# Match the VALUE, not the key: the Stop payload always contains
# the key ("stop_hook_active": false on the first invocation), so
# grepping for the key alone exits 0 every time and the hook below
# never runs.
