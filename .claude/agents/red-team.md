...
CLI project whose tools emit structured JSON and run inside
Claude Code hooks, status lines, and slash commands. Pay
particular attention to anything that reads files, spawns
processes, or parses untrusted on-disk data.
Run `git diff --cached` yourself to see the staged
